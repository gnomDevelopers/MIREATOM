<template>
  <article class="grid grid-cols-12 w-full items-center rounded-lg px-2 py-1 cursor-default border border-solid border-gray-300 bg-gray-100">
    ...
    <div class="flex flex-row justify-end gap-x-2 items-center col-span-2">
      ...
      <div @click="downloadArticle" class="flex flex-col items-center justify-center p-2 btn cursor-pointer rounded-lg">
        <svg class="w-7 h-7" viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
...
import { API_Article_Get_File } from '@/api/api';
...
export default {
  ...
  methods: {
    // API_Article_Get_File
    showArticleFormuls(){
      this.blurStore.showBlur = true;
      this.articleStore.showArticleFormulsMW = true;
    },
    downloadArticle(){
      API_Article_Get_File(this.id)
      .then((response: any) => {
        // Step 1. Attempt base64 decoding and catch exceptions
        ...
        try {
          ...
          // If base64 fails, handle raw binary data
          console.error('Base64 decoding failed, assuming binary data: ', error);
          // Step 2. Convert the string as an array of bytes.
          const bytes = new Uint8Array(data.length);
          for (let i = 0; i < data.length; i++) {
            bytes[i] = data.charCodeAt(i) & 0xFF; // Ensure correct bytes
          }
          // 3. Create a Blob
          const blob = new Blob([bytes], { type: 'application/octet-stream' });
          // 4. Create a Download URL
